        ...
        >
            <div class="aside-top">
                <a class="logo" href="http://localhost:8787" target="_blank">
                    <img :src="logoImg" alt="Temimi Logo">
                </a>
            </div>
...
import logoImg from '@/assets/img/temimi-pink.png';
...
export default {
    ...
    data() {
        return {
            ...
            maskingOpacity: 0,
            active: "", // 侧栏活动项
            path: ["/home", "/content/carousel", "/content/hot-search", "/content/ranking", "/review/video",
            "/review/article", "/review/avatar", "/review/dynamic", "/system/role", "/system/user"],   // 用于判断当前活动页
            defOpenMenu: ["/content", "/review", "/system"],    // 默认展开的子菜单
        }
    },
    computed: {
        user() {
            const u = this.userStore.user || {};
            // 兼容 avatar_url 和 avatar 字段，并提供默认头像
            if (!u.avatar_url && u.avatar) {
                u.avatar_url = u.avatar;
            }
            if (!u.avatar_url) {
                u.avatar_url = 'https://cube.elemecdn.com/3/7c/3ea6beec64369c2642b92c6726f1epng.png';
            }
            return u;
        },
    },
