...
const http = axios.create({
  baseURL: import.meta.env.VITE_API_BASE || '',  // 使用空字符串让Vite代理生效
  timeout: 30000,
    ...
    if (error.response) {
      ...
      switch (status) {
        case 401:
          ...
          // 只有在有 token 的情况下才清除登录状态（说明 token 真的过期了）
          const hasToken = localStorage.getItem('teri_token')
          if (hasToken && (headers.message === 'not login' || data?.message?.includes('登录') || data?.message?.includes('Token'))) {
            userStore.initData()
            ...
            // 没有 token，不显示错误（可能是访问需要登录的接口）
            console.log('访问需要登录的接口，但未登录')
          } else {
