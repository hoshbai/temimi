    ...
    export default {
        ...
        methods: {
            ...
            getCarousels() {
                ...
                this.originalCarousels = carouselJson.slice();
                // 使用 Pinia 更新 store
                if (this.videoStore) {
                    this.videoStore.updateCarousels(this.carousels.slice());
                }
            },
            ...
            changePoint(index) {
                ...
                clearTimeout(timer);
                this.current = index;
                // 创建一个原数组的副本（使用本地保存的原始数据）
                let list = this.originalCarousels.slice();
                if (index == 1) {
