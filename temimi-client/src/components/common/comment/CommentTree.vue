                                ...
                                <div class="reply-operation">
                                    ...
                                    <ul class="operation-list" :style="{ display: showOption ? '' : 'none' }">
                                        <li class="operation-option"
                                            v-if="userStore.userId === rootComment.user.uid || userStore.userId === upUid"
...
export default {
    ...
    methods: {
        ...
        async getCommentTree() {
            if (!this.hasMore || this.loading || (!this.userStore.isLogin && this.commentList.length > 0)) return;
            this.loading = true;
        ...
        // TODO: Implement like/dislike in comment store
        isLike(id) {
            return this.commentStore.likeComment.includes(id);
        },
        isDislike(id) {
            return this.commentStore.dislikeComment.includes(id);
        },
        subLikeComment(id) {
            const index = this.commentStore.likeComment.indexOf(id);
            if (index !== -1) {
                this.commentStore.likeComment.splice(index, 1);
            }
        },
        addLikeComment(id) {
            if (!this.commentStore.likeComment.includes(id)) {
                this.commentStore.likeComment.push(id);
            }
        },
        subDislikeComment(id) {
            const index = this.commentStore.dislikeComment.indexOf(id);
            if (index !== -1) {
                this.commentStore.dislikeComment.splice(index, 1);
            }
        },
        addDislikeComment(id) {
            if (!this.commentStore.dislikeComment.includes(id)) {
                this.commentStore.dislikeComment.push(id);
            }
        },
        ...
        // 获取用户的点赞点踩列表
        async getUserLikes() {
            if (!this.userStore.userId || !this.commentInfo.vid || this.commentInfo.vid === -1) {
                return;
            }

            try {
                const res = await this.$get("/comment/user-likes", {
                    params: { vid: this.commentInfo.vid }
                });

                if (res.data && res.data.code === 200) {
                    this.commentStore.updateLikeComment(res.data.data.likes || []);
                    this.commentStore.updateDislikeComment(res.data.data.dislikes || []);
                }
            } catch (error) {
        ...
        // 触发登录
        toLogin() {
            // TODO: Implement openLogin dialog trigger
            ElMessage.warning('请先登录');
        }
    },
    mounted() {
        // 初始化视频ID
        this.commentInfo.vid = this.$route.params.vid;
        this.getCommentTree();
        // 获取用户的点赞点踩列表
