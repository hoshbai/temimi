    ...
    <div class="comment-textarea">
        <div class="box-normal">
            <div class="reply-box-avatar">
                <div class="temimi-avatar">
                    <VAvatar :img="userStore.user?.avatar_url || ''" :size="isWideWindow ? 48 : 40" :auth="userStore.user?.auth || 0" />
                </div>
            </div>
...
export default {
    ...
    methods: {
        ...
        async sendComment() {
            ...
            // 严格验证所有必需参数
            if (!this.commentInfo.vid || this.commentInfo.vid === -1) {
                ElMessage.error("发送评论失败，请刷新页面后重试");
                return;
            }

            // 确保所有参数都是有效的数字，不是undefined
            const vid = parseInt(this.commentInfo.vid);
            const rootId = parseInt(this.commentInfo.root_id) || 0;
            const parentId = parseInt(this.commentInfo.parent_id) || 0;
            const toUserId = parseInt(this.commentInfo.to_user_id) || 0;

            if (isNaN(vid)) {
                ElMessage.error("视频ID无效，请刷新页面后重试");
                return;
            }

            const formData = new FormData();
            formData.append("vid", vid.toString());
            formData.append("root_id", rootId.toString());
            formData.append("parent_id", parentId.toString());
            formData.append("to_user_id", toUserId.toString());
