                        ...
                        <li class="dm-item" v-for="(item, index) in dmList" :key="index">
                            <div class="dm-info-row" @dblclick="handleDblClick(item.timePoint)">
                                ...
                                <span class="dm-info-delete-btn"
                                    v-if="userStore.userId && (authorId === userStore.userId || item.uid === userStore.userId)"
...
export default {
    ...
    methods: {
        ...
        async deleteDanmu(id) {
            ...
            if (!res.data) return;
            // 立即从本地列表删除（不等WebSocket消息）
            this.danmuStore.removeDanmu(id);
            ElMessage.info("删除成功");
        },
    ...
    watch: {
        // ✅ 修复：监听danmuStore中的弹幕列表变化
        "danmuStore.danmuList": {
            handler(curr) {
