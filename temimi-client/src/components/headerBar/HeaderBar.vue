<template>
    <div class="header-bar" :class="isFixHeaderBar ? 'slide-down' : ''">
        ...
        <div class="left-entry">
            ...
            >
                <picture class="logo">
                    <img src="~assets/img/temimi-pink.png" alt="">
                </picture>
                <span>首页</span>
        ...
        <div class="right-entry">
                ...
                <div class="v-popover to-bottom">
                    <div class="avatar-panel-popover" :class="isPopoverShow ? 'popShow' : 'popHide'" :style="{ display: popoverDisplay }">
                        ...
                        <div class="coins">
                            ...
                            <span class="coins-num">{{ formatCoins(user.coin) }}</span>
                        </div>
                        <div class="counts">
            ...
            <div class="v-popover-wrap">
                <VPopover pop-style="padding-top: 17px;">
                    <template #reference>
                        ...
                        <div class="right-entry--outside" @click="isLogin ? openNewPage('/message') : dialogVisible = true;">
                            <i class="iconfont icon-xinfeng"></i>
                            <span>消息</span>
                        </div>
                    </template>
                    <template #content>
                        <div class="message-entry-popover" v-if="isLogin">
                            <div class="message-inner-list">
                                <div class="message-inner-list__item" @click="openNewPage('/message/reply')">
                                    回复我的
                                    <span class="notify-number" v-if="userStore.msgUnread && userStore.msgUnread[0] > 0">
                                        {{ userStore.msgUnread[0] <= 99 ? userStore.msgUnread[0] : '99+' }}
                                    </span>
                                </div>
                                <div class="message-inner-list__item" @click="openNewPage('/message/at')">
                                    @ 我的
                                    <span class="notify-number" v-if="userStore.msgUnread && userStore.msgUnread[1] > 0">
                                        {{ userStore.msgUnread[1] <= 99 ? userStore.msgUnread[1] : '99+' }}
                                    </span>
                                </div>
                                <div class="message-inner-list__item" @click="openNewPage('/message/love')">
                                    收到的赞
                                    <span class="notify-number" v-if="userStore.msgUnread && userStore.msgUnread[2] > 0">
                                        {{ userStore.msgUnread[2] <= 99 ? userStore.msgUnread[2] : '99+' }}
                                    </span>
                                </div>
                                <div class="message-inner-list__item" @click="openNewPage('/message/system')">
                                    系统消息
                                    <span class="notify-number" v-if="userStore.msgUnread && userStore.msgUnread[3] > 0">
                                        {{ userStore.msgUnread[3] <= 99 ? userStore.msgUnread[3] : '99+' }}
                                    </span>
                                </div>
                                <div class="message-inner-list__item" @click="openNewPage('/message/whisper')">
                                    我的消息
                                    <span class="notify-number" v-if="userStore.msgUnread && userStore.msgUnread[4] > 0">
                                        {{ userStore.msgUnread[4] <= 99 ? userStore.msgUnread[4] : '99+' }}
                                    </span>
                                </div>
                            </div>
                        </div>
            ...
            <div class="v-popover-wrap">
                <VPopover pop-style="padding-top: 17px;">
                    <template #reference>
                        <div class="right-entry--outside" @click="isLogin ? noPage() : dialogVisible = true;">
                            <i class="iconfont icon-fengche"></i>
                            <span>动态</span>
                        </div>
                    </template>
                    <template #content>
                        <div style="height: 557.3px; width: 371.6px;" v-if="isLogin">
            ...
            <div class="v-popover-wrap">
                <VPopover :pop-style="isLogin ? 'padding-top: 17px; margin-left: -200px;' : 'padding-top: 17px;'">
                    <template #reference>
                        <div class="right-entry--outside" @click="isLogin ? openNewPage(`/space/${user.uid}/favlist`) : dialogVisible = true;">
                            <i class="iconfont icon-shoucang"></i>
                            <span>收藏</span>
                        </div>
                    </template>
                    <template #content>
                        <FavoritePopover v-if="isLogin" />
                        <div class="not-login" v-else>
            ...
            <div class="v-popover-wrap">
                <VPopover :pop-style="isLogin ? 'padding-top: 17px; margin-left: -100px;' : 'padding-top: 17px;'">
                    <template #reference>
                        <div class="right-entry--outside" @click="isLogin ? openNewPage(`/space/${user.uid}/history`) : dialogVisible = true;">
                            <i class="iconfont icon-lishijilu"></i>
                            <span>历史</span>
                        </div>
                    </template>
                    <template #content>
                        <HistoryPopover v-if="isLogin" />
                        <div class="not-login" v-else>
            ...
            <div 
                ...
                @click="isLogin ? openNewPage('/platform') : dialogVisible = true;"
            >
            ...
            <div
                ...
                @click="isLogin ? openNewPage('/platform/upload') : dialogVisible = true;"
            >
...
<script>
    ...
    import VLevel from '@/components/common/UserCard/VLevel.vue';
    import FavoritePopover from './FavoritePopover.vue';
    import HistoryPopover from './HistoryPopover.vue';
    import { ElMessage } from 'element-plus';
    import { handleNum, handleLevel, highlightKeyword, formatCoins } from '@/utils/utils.js';
    ...
    export default {
        ...
        computed: {
            ...
            // 计算消息未读数
            msgUnread() {
                return this.userStore.msgUnread?.reduce((a, b) => a + b, 0) || 0;
            },
            // 是否登录
            isLogin() {
            ...
            // 热搜列表
            trendings() {
                return this.videoStore.trendings || [];
            }
        },
        methods: {
            //////// 请求 ////////
