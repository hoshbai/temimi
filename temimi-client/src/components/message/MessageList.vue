...
<script>
import MessageLoading from '@/components/common/MessageLoading.vue';
...
export default {
    ...
    methods: {
        ...
        async getMoreDetails() {
            ...
            this.loading = true;
            const res = await this.$get("/chat/get-more", {
                params: {
                    anotherId: this.mid,
            ...
            if (res.data && res.data.data) {
                let chat = this.userStore.chatList.find(item => item.user.uid === this.mid);
        ...
        withdraw() {
            ...
            this.userStore.ws.send(JSON.stringify(context));
            this.msgId = -1;
        },

        // 删除消息
        async deleteMsg() {
            const res = await this.$post("/chat/delete-message", { id: this.msgId }, {
                headers: {
    ...
    watch: {
        "mid"(curr) {
            let chat = this.userStore.chatList.find(item => item && item.user && item.user.uid === curr);
            // 复制一份，防止指向同一个地址
            if (chat) {
        ...
        "userStore.chatList": {
            handler(curr) {
                this.$nextTick(() => {
                    const currChat = curr.find(item => item && item.user && item.user.uid === this.mid);
