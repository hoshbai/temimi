...
export default {
    ...
    setup() {
        ...
        return { userStore, videoStore, danmuStore };
    },
    data() {
        return {
            isMounted: false,   // 做个三秒挂载延迟，防止increasePlay函数重复触发
            hasRecordedPlay: false,  // 是否已经记录过播放量（防止重复计数）
            playRecordThreshold: 0.5,  // 播放进度阈值（50%）
            wrapSize: {
                width: 704,
                height: 442,
            },
    ...
    methods: {
        ...
        displayDanmus(currTimePoint) {
            ...
            while(this.dmIndex + 1 < this.dmList.length && this.dmList[this.dmIndex + 1].timePoint <= currTimePoint) {
                ...
                if (dm.timePoint > this.lastTimePoint) {
                    ...
                    // 标识自己发的弹幕
                    if (this.userStore.userId && dm.uid === this.userStore.userId) {
        ...
        timeUpdate() {
            ...
            // ✅ 新增：当观看进度达到50%时，记录一次播放量
            if (!this.hasRecordedPlay && this.duration > 0) {
                const watchProgress = time / this.duration;
                if (watchProgress >= this.playRecordThreshold) {
                    this.hasRecordedPlay = true;
                    this.increasePlay();
                    console.log('播放进度达到50%，记录播放量');
                }
            }
        ...
        // 视频播放结束
        ended() {
            // ✅ 新增：如果视频播放完成但还没记录播放量（比如视频很短，不到50%就结束了），也记录一次
            if (!this.hasRecordedPlay) {
                this.hasRecordedPlay = true;
                this.increasePlay();
                console.log('视频播放完成，记录播放量');
            }
