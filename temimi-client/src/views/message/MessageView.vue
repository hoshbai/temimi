                    ...
                    <div class="side-bar">
                        ...
                        <ul class="side-list">
                            ...
                            >
                                <div class="text">
                                    {{ item.name }}
                                    <span class="notify notify-number" v-if="userStore.user && userStore.user.uid && userStore.msgUnread && userStore.msgUnread[index] > 0">
                                        {{ userStore.msgUnread[index] <= 99 ? userStore.msgUnread[index] : '99+' }}
                                    </span>
                                </div>
                            </li>
                        </ul>
...
export default {
    ...
    beforeRouteEnter(to, from, next) {
        // 设置全局背景
        // 使用导入的图片路径
        document.body.style.cssText = `background: url(${messageBg}) top/cover no-repeat fixed;`;
