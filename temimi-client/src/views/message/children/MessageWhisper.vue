                    ...
                    <div class="list">
                        ...
                        >
                            ...
                            <div class="name-box">
                                ...
                                <div class="last-word" v-if="item.detail && item.detail.list && item.detail.list.length > 0">
                                    {{ item.detail.list[item.detail.list.length - 1].withdraw === 1 ? `${item.detail.list[item.detail.list.length - 1].userId === user.uid ? '你' : '对方'}撤回了一条消息` : item.detail.list[item.detail.list.length - 1].content }}
                                </div>
                            </div>
                            <div class="close">
                            ...
                            <div class="notify notify-number" v-if="item.chat && item.chat.unread > 0">{{ item.chat.unread }}</div>
                        </div>
                    </div>
                </div>
            </div>
...
export default {
    ...
    computed: {
        chatList() {
            // 过滤掉没有 user 属性的项
            return this.userStore.chatList.filter(item => item && item.user);
        },
        chatId() {
            return this.userStore.chatId;
        },
        user() {
    ...
    methods: {
        ...
        async getChatList() {
            ...
            this.loading = true;
            const res = await this.$get("/chat/recent-list", {
                params: {
                    offset: this.userStore.chatList.length
                },
                headers: {
            ...
            if (res.data && res.data.data) {
                ...
                this.userStore.updateChatList(res.data.data.list);
            }
            this.$nextTick(() => {
                this.loading = false;
            });
        ...
        // 创建聊天
        async createChat() {
            const res = await this.$get(`/chat/create/${this.$route.params.mid}`, {
                headers: {
                    Authorization: "Bearer " + localStorage.getItem("teri_token"),
                },
            });
            if (res.data.code === 200) {
                if (res.data.data) {
                    // 新创建
                    this.userStore.chatList.unshift(res.data.data);
                }
                this.userStore.chatId = Number(this.$route.params.mid);
            } else {
                this.userStore.chatId = -1;
            }
        },

        // 移除聊天
        async closeChat(uid) {
            let i = this.userStore.chatList.findIndex(item => item.user && item.user.uid === uid);
            if (i !== -1) {
                this.userStore.chatList.splice(i, 1);    // 移除这个元素
            }
            if (Number(this.$route.params.mid) === uid) {
                // 如果移除的是当前聊天 就打开第一个聊天
                if (this.userStore.chatList.length > 0 && this.userStore.chatList[0].chat) {
                    this.changeChat(this.userStore.chatList[0].chat.userId);
                } else {
                    this.userStore.chatId = -1;
                }
            }
            await this.$get(`/chat/delete/${uid}`, {
                headers: {
                    Authorization: "Bearer " + localStorage.getItem("teri_token"),
                },
            });
    ...
    async mounted() {
        this.userStore.isChatPage = true;
        await this.getChatList();
    ...
    beforeUnmount() {
        this.userStore.isChatPage = false;
    },
    watch: {
        // 监听路由变化打开对应聊天
        "$route.path"() {           
