...
export default {
    ...
    methods: {
        ...
        // 更新对方用户 以及缓存自己的id
        async updateUser() {
            let i = this.userStore.chatList.findIndex(item => item && item.user && item.user.uid === this.mid);
            if (i !== -1 && this.userStore.chatList[i]) {
                this.user = this.userStore.chatList[i].user || { uid: this.mid, nickname: '未知用户' };
                this.myId = this.userStore.chatList[i].chat?.anotherId;
            } else {
                // 如果找不到，主动获取用户信息
                try {
                    const res = await this.$get(`/user/profile/${this.mid}`);
                    if (res.data && res.data.code === 200 && res.data.data) {
                        ...
                        this.user = { uid: this.mid, nickname: '用户' + this.mid };
                    }
                } catch (error) {
                    console.warn('获取用户信息失败，使用默认值', error);
                    this.user = { uid: this.mid, nickname: '用户' + this.mid };
                }
                this.myId = this.userStore.user.uid;
            }
        },

        // 更新窗口在线状态
        async updateOnline() {
            await this.$get("/chat/online", {
                params: { fromUid: this.user.uid },
        ...
        // 更新聊天窗口离开状态
        async updateOutline() {
            await this.$get("/chat/outline", {
                params: { fromUid: this.user.uid, toUid: this.myId }
            })
        },
        ...
        sendMsg() {
            ...
            this.userStore.ws.send(JSON.stringify(msg));
            // 清空文本
