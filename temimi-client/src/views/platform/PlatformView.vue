<template>
    <div class="platform-view">
        ...
        <div class="platform-header">
            <div class="header-content">
                ...
                <div class="right-block">
                    ...
                    <VPopover pop-style="padding-top: 10px;" v-else-if="user && user.uid">
                        <template #reference>
                            <div class="avatar">
                                <img :src="user.avatar_url || user.avatar" :alt="`${user.nickname || '用户'}的头像`">
                            </div>
                        </template>
                        <template #content>
        ...
        <!-- 主体 -->
        <div class="platform-main" v-if="!isLoading">
            <div class="content-body" v-if="userStore.isLogin">
                <router-view></router-view>
            </div>
            <div class="not-login-placeholder" v-else>
                <div>请先登录</div>
            </div>
        </div>
        <div class="platform-main" v-else>
...
export default {
    ...
    computed: {
        ...
        msgUnread() {
            let count = 0;
            if (this.userStore.msgUnread && Array.isArray(this.userStore.msgUnread)) {
                for (var i = 0; i < 5; i++) {
                    count += this.userStore.msgUnread[i] || 0;
                }
            }
            return count;
        }
    ...
    async created() {
        ...
        if (token && !this.userStore.isLogin) {
...
.loading-placeholder, .not-login-placeholder {
    display: flex;
    justify-content: center;
    align-items: center;
    height: 100%;
