        ...
        <div class="space-container">
            ...
            <div class="h">
                <div class="wrapper">
                    ...
                    <div class="h-inner" :style="user.bg_url ? `background-image: url('${user.bg_url.startsWith('http') ? user.bg_url : 'http://localhost:8080' + user.bg_url}');` : ''">
                        <div class="h-gradient"></div>
                        <div class="h-user">
                            <div class="h-info">
                                <div class="avatar-container">
                                    <VAvatar :img="user.avatar_url" :size="60" :auth="user.auth"></VAvatar>
                                    <a v-if="user.uid === this.userStore.user.uid" href="/account/avatar" target="_blank" class="avatar-cover">更换头像</a>
                                </div>
                                <div class="h-basic">
                                    <div class="basic-top">
                        ...
                        <div class="h-action" v-if="user.uid !== this.userStore.user.uid">
                            <div class="h-f-btn h-follow" :class="{'followed': isFollowing}" @click="toggleFollow">
                                {{ isFollowing ? '已关注' : '关注' }}
                            </div>
                        ...
                        <div title="更换皮肤" class="space-theme-trigger icon" v-if="user.uid === this.userStore.user.uid"
                    ...
                    <!-- 更换主题面板 -->
                    <div class="space-theme" :class="{'theme-show': themeShow}" v-if="user.uid === this.userStore.user.uid">
                        ...
                        <div class="theme-panel">
                            <div class="wrapper">
                                <div class="theme-upload">
                                    ...
                                    <div class="upload-btn" @click="$refs.bgUpload.click()">
                                        <i class="iconfont icon-shangchuan"></i>
                                        <span>上传自定义背景</span>
                                    </div>
                                    <p class="upload-tip">支持 JPG、PNG 格式，文件大小不超过 20MB</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="n-inner">
                        ...
                        <div class="n-tab-links" @mouseleave="hoverIdx = -1">
                            ...
                            <div class="n-btn n-favlist" :class="{'active': navIdx === 4}" @click="clickNavItem('/favlist')" @mouseenter="hoverIdx = 4">
                                ...
                                <span class="n-num">{{ this.favoriteStore.userFavList.length }}</span>
                            </div>
                            <div class="n-btn n-setting" v-if="this.userStore.user.uid === user.uid" :class="{'active': navIdx === 5}" @click="clickNavItem('/setting')" @mouseenter="hoverIdx = 5">
...
export default {
    ...
    setup() {
        ...
        return { userStore, videoStore, favoriteStore };
    },
    data() {
        return {
            user: {},   // 用户信息
    ...
    computed: {
        ...
        barLeft() {
            ...
            if (!this.mounted) return 0;
            const element = document.querySelector(target);
            const wrapper = document.querySelector(".n-wrapper");
            if (!element || !wrapper) return 0;
            return element.getBoundingClientRect().left - wrapper.getBoundingClientRect().left;
        },
    ...
    methods: {
        ...
        async getUserInfo(uid) {
            try {
                const data = await this.userStore.fetchUserProfile(uid);
                if (!data) {
        ...
        async getUserFavList(uid) {
            try {
                const data = await this.userStore.fetchUserFavorites(uid);
                if (data) {
                    // 将默认置顶
                    ...
                    this.favoriteStore.setUserFavList(list);
                }
            } catch (error) {
        ...
        async getUserWorksCount(uid) {
            try {
                const data = await this.videoStore.fetchUserWorksCount(uid);
                if (data !== undefined) {
                    this.worksCount = data;
                }
            } catch (error) {
                console.error('Failed to fetch user works count:', error);
            }
        },
        ...
        async handleBgUpload(event) {
            ...
            try {
                ...
                if (res.data && res.data.code === 200) {
                    this.user.bg_url = res.data.data;
        ...
        async checkFollowStatus() {
            if (!this.userStore.isLogin || !this.user.uid) return;
            try {
                const res = await this.$get(`/user/follow/status/${this.user.uid}`);
                if (res.data?.data) {
                    this.isFollowing = res.data.data.isFollowing;
                }
            } catch (e) {
        ...
        async toggleFollow() {
            ...
            try {
                ...
                const res = await this.$post(`/user/follow/${this.user.uid}?action=${action}`);
                if (res.data?.code === 200) {
                    ...
                    if (this.isFollowing) {
                        ...
                        this.user.fansCount = Math.max(0, (this.user.fansCount || 0) - 1);
                    }
    ...
    async created() {
        if (this.$route.path === "/space" || this.$route.path === "/space/") {
            if (this.userStore.token) {
                try {
                    // 解析JWT获取当前用户uid
                    const w = this.userStore.token.split('.')[1];
