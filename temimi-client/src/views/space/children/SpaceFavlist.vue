<template>
    <div class="space-favlist clearfix">
        <div class="fav-sidenav">
            <div class="nav-container">
                ...
                <div class="fav-list-container ps">
                    ...
                    <ul class="fav-list">
                        <li class="fav-item" :class="{'cur': Number(fid) === item.fid, 'owner': item.uid === this.userStore.user.uid}" v-for="(item, index) in favList" :key="index">
                            ...
                            <div class="more fav-dropdown" @click.stop="toggleDropdown(item.fid, $event)">
                                <div class="tool-more">
                                    <i class="iconfont icon-gengduo"></i>
                                </div>
                                <ul class="be-dropdown-menu" v-if="activeDropdown === item.fid" :style="dropdownStyle">
                                    <li class="be-dropdown-item" :class="{'be-dropdown-item-delimiter': item.type !== 1}" @click.stop="noPage">编辑信息</li>
                                    <li class="be-dropdown-item" v-if="item.type !== 1" @click.stop="deleteFav(item.fid)">删除</li>
                                </ul>
                            </div>
                        </li>
                    </ul>
        ...
        <div class="fav-main">
            <div class="favList-info">
                <div class="favInfo-box">
                    <a class="favInfo-cover">
                        <img v-if="favCover" :src="favCover" alt="">
            ...
            <div class="fav-content" v-if="currFav && favVideos.length > 0">
                <ul class="fav-video-list clearfix">
                    <li class="small-item" v-for="(item, index) in favVideos" :key="index">
                        ...
                        <VPopover class="video-edit" popStyle="position: fixed; padding-top: 10px; z-index: 1000; top: auto; left: auto; transform: translate3d(-75%,0,0);">
                            ...
                            <template #content>
                                <ul class="be-dropdown-menu" v-if="this.userStore.user.uid === currFav.uid">
...
export default {
    ...
    setup() {
        ...
        return { userStore, favoriteStore };
    },
    data() {
        return {
            isFavnavOpen: true,
    ...
    computed: {
        ...
        // 收藏夹列表
        favList() {
            return this.favoriteStore.userFavList;
        },

        // 当前收藏夹
        currFav() {
            const fav = this.favList.find(item => item.fid === Number(this.fid));
            if (!fav || (fav.uid !== this.userStore.user.uid && fav.visible === 0)) {
                return null;
            } else {
                return fav;
            }
    ...
    methods: {
        ...
        async getFavVideos() {
            ...
            // ✅ 修复：使用正确的 API 路径
            const res = await this.$get(`/favorite/${this.currFav.fid}/videos`, {
                params: {
                    pageNum: this.page,
                    pageSize: 20
                }
            ...
            if (!res.data || res.data.code !== 200) {
                console.error('获取收藏夹视频失败:', res.data?.message);
                return;
            }
            
            // 后端返回的是 Page<Video> 对象，需要转换为前端期望的格式
            const pageData = res.data.data;
            const videos = pageData.records || [];
            
            // 转换数据格式以匹配前端模板
            this.favVideos = videos.map(video => ({
                video: {
                    vid: video.vid,
                    title: video.title,
                    status: video.status,
                ...
                stats: {
                    play: video.play || 0,
                    collect: video.collect || 0
                },
                user: {
                    nickname: video.authorName || '未知用户'
                },
                info: {
                    vid: video.vid,
                    fid: this.currFav.fid,
                    time: video.uploadDate  // 使用上传时间作为收藏时间的临时替代
                }
            }));
            
            // 更新收藏夹封面：优先使用后端返回的封面，否则使用第一个视频的封面
            if (this.currFav.cover) {
                this.favCover = this.currFav.cover;
            } else if (this.favVideos.length > 0) {
                this.favCover = this.favVideos[0].video.coverUrl;
            } else {
                this.favCover = '';
            }
        ...
        // 删除收藏夹
        async deleteFav(fid) {
            this.activeDropdown = null; // 关闭下拉菜单
            try {
                const res = await this.$axios.delete(`/api/favorite/delete/${fid}`, {
                    headers: { Authorization: "Bearer " + localStorage.getItem("teri_token") }
                });
                
                if (!res.data || res.data.code !== 200) {
                    ElMessage.error(res.data?.message || '删除收藏夹失败');
                    return;
                }
                ...
                // 从列表中移除已删除的收藏夹
                this.favoriteStore.setUserFavList(this.favoriteStore.userFavList.filter(item => item.fid !== fid));

                // 如果删除的是当前收藏夹，跳转到默认收藏夹
                if (Number(this.fid) === fid) {
                    const defaultFav = this.favoriteStore.userFavList.find(item => item.type === 1);
                    if (defaultFav) {
                        this.$router.push(`/space/${this.uid}/favlist?fid=${defaultFav.fid}`);
                    } else if (this.favoriteStore.userFavList.length > 0) {
                        this.$router.push(`/space/${this.uid}/favlist?fid=${this.favoriteStore.userFavList[0].fid}`);
                    }
                }
            } catch (error) {
                console.error('删除收藏夹失败:', error);
                const errorMsg = error.response?.data?.message || error.message || '删除收藏夹失败，请稍后重试';
                ElMessage.error(errorMsg);
            }
        },

        // 切换下拉菜单
        toggleDropdown(fid, event) {
            if (this.activeDropdown === fid) {
                this.activeDropdown = null;
            } else {
                this.activeDropdown = fid;
                // 计算下拉菜单的位置
        ...
        handleClickOutside(event) {
            const dropdown = document.querySelector('.fav-dropdown');
