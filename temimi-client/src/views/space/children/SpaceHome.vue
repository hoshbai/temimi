        ...
        <div class="col-1">
            ...
            <div class="section video">
                <h3 class="section-title">
                    <span class="t" @click="$router.push(`/space/${uid}/video`)">{{ userStore.user.uid === uid ? '我' : 'TA' }}的视频</span>
            ...
            <div class="section article">
                <h3 class="section-title">
                    <span class="t" @click="$router.push(`/space/${uid}/article`)">{{ userStore.user.uid === uid ? '我' : 'TA' }}的专栏</span>
            ...
            <div class="section fav">
                <h3 class="section-title">
                    <span class="t" @click="$router.push(`/space/${uid}/favlist`)">{{ userStore.user.uid === uid ? '我' : 'TA' }}的收藏夹</span>
                ...
                <div class="fav-content">
                    <div v-for="(item, index) in favList" :key="index">
                        <div class="fav-item" v-if="item.visible === 1 || item.uid === userStore.user.uid">
        ...
        <div class="col-2">
            <div class="section user-info">
                <p class="user-info-title">
                    ...
                    <a href="/account/info" target="_blank" v-if="uid === userStore.user.uid">
...
export default {
    ...
    methods: {
        ...
        // 获取用户收藏夹
        async getFavList() {
            const res = await this.$get(`/favorite/user/${this.uid}`);
            if (!res.data) return;
            // 将默认置顶
        ...
        // 获取用户最近投币的视频
        async getCoinVideos() {
            const res = await this.$get("/video/user-coin", {
                params: {
                    uid: this.uid,
                    offset: 0,
                    quantity: 20
