...
**设计理由**:
1. **精确定位**: 记录弹幕出现的精确时间点，精确到0.01秒
2. **查询优化**: 播放器加载时查询某个时间段的弹幕，需要按时间点查询
3. **联合索引**: (vid, time_point)联合索引，支持高效范围查询
4. **DECIMAL类型**: 避免浮点数精度问题

**查询示例**:
```sql
-- 查询视频0-60秒的弹幕
SELECT * FROM danmu 
WHERE vid = 1 AND time_point BETWEEN 0 AND 60 
ORDER BY time_point;
```

**如果老师问**: "为什么不用INT存储毫秒？"
**回答**: "可以用INT存储毫秒，但DECIMAL(10,2)更直观，直接对应视频播放时间。而且10位整数+2位小数足够表示99999999.99秒（约3年），完全够用。如果用INT存毫秒，需要在应用层转换，增加复杂度。"

---

#### Q: 为什么评论表要有parent_id和root_id两个字段？
...
**设计理由**:
1. **parent_id**: 指向直接父评论，用于构建树形结构
   - 根评论的parent_id = 0
   - 回复评论的parent_id = 被回复评论的ID

2. **root_id**: 指向根评论，用于快速查询整个评论树
   - 根评论的root_id = 0
   - 所有子孙评论的root_id = 根评论ID

**查询优化**:
```sql
-- 查询某个根评论的所有回复（一次查询，避免递归）
SELECT * FROM comment WHERE root_id = 123;

-- 查询某个评论的直接回复
SELECT * FROM comment WHERE parent_id = 456;
```

**如果老师问**: "为什么不只用parent_id递归查询？"
**回答**: "递归查询性能差，需要多次查询数据库。有了root_id，一次查询就能获取整个评论树的所有节点，然后在应用层构建树形结构。这是典型的空间换时间策略。"

**树形结构示例**:
```
根评论 (id=1, parent_id=0, root_id=0)
├─ 回复1 (id=2, parent_id=1, root_id=1)
│  └─ 回复1-1 (id=3, parent_id=2, root_id=1)
└─ 回复2 (id=4, parent_id=1, root_id=1)
```
...
**设计理由**:
1. **查询性能**: 避免每次都COUNT，直接读取字段
2. **排序需求**: 按播放量、点赞数排序，需要这些字段
3. **高频访问**: 视频列表页需要显示这些数据，访问频率极高
4. **实时性要求不高**: 统计数据允许短暂延迟
...
-- 性能差，每次都要扫描整个表
SELECT v.*, 
       (SELECT COUNT(*) FROM user_video_like WHERE vid = v.vid) as like_count,
       (SELECT COUNT(*) FROM danmu WHERE vid = v.vid) as danmu_count
FROM video v;
```
...
// 用户点赞时更新
@Transactional
public void likeVideo(Integer vid, Integer uid) {
    // 1. 插入点赞记录
    userVideoLikeMapper.insert(new UserVideoLike(vid, uid));
    
    // 2. 更新视频点赞数
    videoMapper.incrementLikeCount(vid);
}
```

**如果老师问**: "如果更新失败，数据会不一致怎么办？"
**回答**: "使用@Transactional事务保证原子性，要么都成功，要么都失败。如果担心高并发下的计数不准确，可以定期用定时任务校准，或者使用Redis计数器。"

---

#### Q: 为什么要建立联合索引(vid, time_point)？
...
**设计理由**:
1. **最左前缀原则**: 可以用于WHERE vid=? 或 WHERE vid=? AND time_point>?
2. **覆盖索引**: 查询只需要这两个字段时，不需要回表
3. **排序优化**: ORDER BY time_point时，索引已经有序

**查询场景**:
```sql
-- 场景1: 查询某视频的所有弹幕（用到vid）
SELECT * FROM danmu WHERE vid = 1;

-- 场景2: 查询某视频某时间段的弹幕（用到vid和time_point）
SELECT * FROM danmu 
WHERE vid = 1 AND time_point BETWEEN 0 AND 60;

-- 场景3: 查询某视频的弹幕并按时间排序（用到vid和time_point）
SELECT * FROM danmu WHERE vid = 1 ORDER BY time_point;
```

**如果老师问**: "为什么不分别建两个单列索引？"
**回答**: "单列索引只能用一个，MySQL会选择其中一个索引。联合索引可以同时利用两个字段，而且遵循最左前缀原则，WHERE vid=? 也能用到这个索引。联合索引比两个单列索引更高效。"

---

### 11.6 前端组件设计细节

#### Q: 为什么视频播放器要自己实现而不用现成的？
...
**设计理由**:
1. **定制需求**: 需要集成弹幕功能，现成播放器难以深度定制
2. **UI统一**: 自定义UI与网站风格统一
3. **功能控制**: 完全控制播放逻辑，添加自定义功能
4. **学习价值**: 深入理解HTML5 Video API

**如果老师问**: "为什么不用Video.js？"
**回答**: "Video.js是很好的播放器库，但我们需要深度集成弹幕系统，弹幕要与视频时间精确同步，需要完全控制播放器。而且自己实现可以学习Video API的原理，对技术成长有帮助。如果是生产环境，可以考虑基于Video.js扩展。"
...
```javascript
@loadedmetadata="videoCanPlay"  // 元数据加载完成
@timeupdate="timeUpdate"        // 播放时间更新
@progress="updateBufferingBar"  // 缓冲进度更新
@waiting="buff = true"          // 缓冲中
@canplay="buff = false"         // 可以播放
@ended="ended"                  // 播放结束
```

**设计理由**:
1. **loadedmetadata**: 获取视频时长、分辨率等信息
2. **timeupdate**: 更新进度条，同步弹幕
3. **progress**: 显示缓冲进度（灰色进度条）
4. **waiting/canplay**: 显示/隐藏加载动画
5. **ended**: 播放结束后的处理（循环播放、推荐视频等）

**如果老师问**: "timeupdate触发频率是多少？"
**回答**: "timeupdate大约每250ms触发一次，频率不固定。如果需要更精确的时间控制，可以用requestAnimationFrame自己实现。但对于进度条和弹幕来说，250ms的精度已经足够。"
...
.danmu-item {
  position: absolute;
  white-space: nowrap;
  animation: danmu-scroll 10s linear;
}
...
**设计理由**:
1. **层叠显示**: 多条弹幕可以重叠显示
2. **动画流畅**: CSS动画性能好，GPU加速
3. **不影响布局**: 不占据文档流，不影响视频播放器布局
4. **精确定位**: 可以精确控制弹幕位置（顶部、底部、滚动）

**如果老师问**: "为什么不用JavaScript控制位置？"
**回答**: "CSS动画由GPU加速，性能比JavaScript好。而且CSS动画声明式，代码更简洁。JavaScript适合复杂的交互逻辑，简单的移动动画用CSS更合适。"
...
// 图片懒加载
<img v-lazy="video.cover_url" />
```

**设计理由**:
1. **首屏加载快**: 只加载当前页面需要的代码
2. **按需加载**: 用户访问时才加载对应页面
3. **减少带宽**: 不访问的页面不下载
4. **代码分割**: Vite自动将懒加载的组件打包成独立文件
...
├── index.html
├── assets/
│   ├── index-abc123.js      (主包，约100KB)
│   ├── VideoDetail-def456.js (懒加载，约50KB)
│   └── UserSpace-ghi789.js   (懒加载，约30KB)
```

**如果老师问**: "懒加载会不会导致切换页面时卡顿？"
**回答**: "首次访问时会有短暂加载，但可以用预加载优化。而且现代浏览器会缓存已加载的文件，第二次访问就很快了。对于首屏性能的提升，这点延迟是值得的。"
...
**设计理由**:
1. **防抖(debounce)**: 用户停止输入后才搜索，避免频繁请求
2. **节流(throttle)**: 限制滚动事件处理频率，避免性能问题
3. **用户体验**: 减少不必要的请求，提升响应速度
4. **服务器压力**: 减少服务器负载

**区别**:
- **防抖**: 等待停止后执行（搜索、输入验证）
- **节流**: 固定时间间隔执行（滚动、resize）

**如果老师问**: "为什么搜索用防抖而不是节流？"
**回答**: "因为用户输入'JavaScript'时，我们不需要搜索'J'、'Ja'、'Jav'...，只需要搜索最终的'JavaScript'。防抖等用户停止输入后才搜索，避免无意义的请求。而滚动需要持续响应，所以用节流。"
...
**设计理由**:
1. **代码简洁**: Controller不需要try-catch，直接抛异常
2. **统一格式**: 所有错误响应格式一致
3. **日志记录**: 统一记录异常日志
4. **安全性**: 避免暴露敏感信息（如数据库错误）
...
```java
@PostMapping("/publish")
public ApiResult publish(@RequestBody Dynamic dynamic) {
    try {
        dynamicService.publish(dynamic);
...
@PostMapping("/publish")
public ApiResult publish(@RequestBody Dynamic dynamic) {
    dynamicService.publish(dynamic);  // 直接抛异常，由全局处理
    return ApiResult.success();
}
```

**如果老师问**: "为什么不在Service层处理异常？"
**回答**: "Service层应该专注业务逻辑，异常处理是横切关注点，应该统一处理。而且Service层的异常可能需要不同的HTTP状态码，在Controller层处理更合适。全局异常处理器就是AOP思想的体现。"
...
**示例**:
- **老师**: "这里为什么用CompletableFuture？"
- **你**: "这段代码我还没有深入研究，但从代码结构看，应该是为了异步处理，避免阻塞主线程。CompletableFuture是Java 8引入的异步编程工具，比传统的Future更强大。如果让我实现，我会考虑用它来处理耗时操作，比如发送邮件、调用第三方API等。"
...
**示例**:
- **老师**: "为什么不用Redis缓存？"
- **你**: "Redis确实是很好的缓存方案，可以显著提升性能。当前项目没有引入Redis主要考虑：1) 项目规模较小，数据库性能暂时够用；2) 增加Redis会增加部署复杂度；3) 学习时间有限。但我了解Redis的使用场景，比如缓存热点数据、分布式锁、消息队列等。如果项目需要，我可以快速学习并集成。"
...
**示例**:
- **老师**: "这个查询在数据量大时会很慢吧？"
- **你**: "您说得对，当前实现在数据量大时确实会有性能问题。可以从几个方面优化：1) 添加索引；2) 分页查询，避免一次加载全部数据；3) 使用缓存；4) 数据库读写分离。如果是生产环境，我会先用EXPLAIN分析查询计划，然后针对性优化。"
...
**示例**:
- **老师**: "这里有SQL注入风险吗？"
- **你**: "我使用了MyBatis-Plus的参数化查询，所有参数都是通过占位符传递的，不是字符串拼接，所以不存在SQL注入风险。MyBatis-Plus会自动处理参数转义。如果是原生SQL，我会特别注意使用#{param}而不是${param}。"
...
#### 1. 开场（1分钟）
"各位老师好，我今天展示的是Temimi视频平台。这是一个仿B站的项目，实现了视频播放、实时弹幕、评论互动等核心功能。"
...
- 发送弹幕，展示实时广播
- 多开窗口，演示多用户
- 说明："WebSocket双向通信，会话池管理，支持多视频并发"
- 展示后端日志："可以看到连接建立、消息接收、广播的日志"

**3.3 代码展示（3分钟）**
- 展示`DanmuWebSocketHandler.java`
- 说明："这是WebSocket处理器，负责连接管理和消息广播"
- 展示`videoSessionMap`
- 说明："使用ConcurrentHashMap管理会话池，按视频ID分组"

**3.4 数据库（2分钟）**
- 展示弹幕表结构
- 说明："time_point字段记录弹幕时间点，联合索引优化查询"
- 展示数据："可以看到刚才发送的弹幕已经保存"

#### 4. 总结（2分钟）
"项目实现了完整的视频平台核心功能，技术亮点包括：
1. WebSocket实时弹幕系统
2. JWT无状态认证
3. 自定义视频播放器
4. 评论树形结构

通过这个项目，我深入学习了前后端开发、实时通信、数据库设计等技术。谢谢各位老师！"
